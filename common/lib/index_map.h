...
struct in_imap {
  ...
  T operator() (const size_t i) {return (*f)(i+s);}
  in_imap<T,F> cut(size_t ss, size_t ee) const {return in_imap<T,F>(s+ss,s+ee,*f); }
...
struct out_imap {
  ...
  out_imap<T,F> cut(size_t ss, size_t ee) const {return out_imap<T,F>(s+ss,s+ee,f); }
...
struct iter_imap {
  ...
  T& operator() (const size_t i) const {return s[i];}
  iter_imap<Iterator> cut(size_t ss, size_t ee) const {
...
public:
  ...
  E& operator() (const size_t i) const {return s[i];}
  array_imap<T> cut(size_t ss, size_t ee) const {
    return array_imap<T>(s + ss, ee - ss);}
  void update(size_t i, const E& val) {s[i] = val;}
  const size_t size() const { return e - s;}
