...
std::vector<aMatrix> compute_lstm(std::vector<aMatrix>& weights,
...
void learn_lstm() {
  ...
  for (int iter = 0; iter < NUM_ITER; ++iter) {
    ...
    for (int i = 0; i < BATCH_SIZE; ++i) {
      ...
      std::vector<Matrix> sample = input[batch_dis(generator)];
      // Run the LSTM on the batch data
      std::vector<aMatrix> output_softmax = compute_lstm(
              *weight_hyper_list[0], sample, NUM_ASCII, HIDDEN_FEATURES);
      // Compute the loss
