...
// tfkdiff class
    ...
    void tfkdiff::get_tls_references() {
      ...
      //if (stack_list.get_reference().size() == 0) {
      //  stack_list.push_back(triple_vector_wl());
      //} else {
      //  //stack_list.get_reference().back().statement_stack_end = worker_local_stacks[thread_local_worker_id].statement_stack_arr_len;
      //  //stack_list.get_reference().back().operation_stack_end = worker_local_stacks[thread_local_worker_id].operation_stack_arr_len;
      //  //stack_list.get_reference().back().multiplier_stack_end = worker_local_stacks[thread_local_worker_id].multiplier_stack_arr_len;
      //  //stack_list.get_reference().back().has_bounds = true;//worker_local_stacks[thread_local_worker_id].multiplier_stack_arr_len;

      //  stack_list.push_back(triple_vector_wl());
      //}
      //printf("stack list view size %d\n", stack_list.get_reference().size());
      //triple_vector_wl ret = stack_list.get_reference().back();
      //;
    }
    ...
    void tfkdiff::clear() {
      ...
      //  delete stacks[i].statement_stack;
      //  delete stacks[i].operation_stack;
      //  delete stacks[i].multiplier_stack;
      //}
      //stack_list.get_reference().clear();
      ...
      //triple_vector_wl val = get_tls_references();
      get_tls_references();

      //thread_local_multiplier = val.multiplier_stack;
      //thread_local_index = val.operation_stack;
      //thread_local_statement = val.statement_stack;
    }

    std::vector<triple_vector_wl> tfkdiff::collect() {
      //std::vector<triple_vector_wl> ret(0);
      //std::list<triple_vector_wl>& stacks = stack_list.get_reference();
      ...
      std::vector<std::vector<std::pair<uint64_t, triple_vector_wl*> > > _wl_ret(__cilkrts_get_nworkers());
      ...
      for (int wid = 0; wid < __cilkrts_get_nworkers(); wid++) {
        ...
        for (int i = 0; i < wl_ret[wid].size()-1; i++) {
          ...
          wl_ret[wid][wl_ret[wid].size()-1]->statement_stack_end = worker_local_stacks[wid].statement_stack_arr_len;
          wl_ret[wid][wl_ret[wid].size()-1]->operation_stack_end = worker_local_stacks[wid].operation_stack_arr_len;
          wl_ret[wid][wl_ret[wid].size()-1]->multiplier_stack_end = worker_local_stacks[wid].multiplier_stack_arr_len;
          wl_ret[wid][wl_ret[wid].size()-1]->gradient_registered_end = worker_local_stacks[wid].gradient_registered_arr_len;
          wl_ret[wid][wl_ret[wid].size()-1]->gradient_unregistered_end = worker_local_stacks[wid].gradient_unregistered_arr_len;
        }
      }
      ...
      //      printf("start %d end %d, steal count %d\n", wl_ret[wid][i]->statement_stack_start, wl_ret[wid][i]->statement_stack_end, wl_ret[wid][i]->steal_count);
      //    }
      //  }
      //}
      //#endif
      //sp_tree.walk_tree_prepare(sp_tree.get_root(), NULL, NULL);
