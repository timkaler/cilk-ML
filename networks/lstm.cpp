...
vector<aMatrix> compute_lstm_par(vector<aMatrix>& weights, vector<Matrix>& input) {
...
// Same as above, without the added parallelism inside LSTM
...
void learn_lstm(bool parallel) {
  ...
  for (int iter = 0; iter < NUM_ITER; ++iter) {
    ...
    cilk_for (int i = 0; i < BATCH_SIZE; ++i) {
      ...
      vector<aMatrix> output_softmax;
      if (parallel) {
        output_softmax = compute_lstm_par(*weight_hyper_list[0], sample);
      } else {
        output_softmax = compute_lstm(*weight_hyper_list[0], sample);
      }
    ...
    // Reverse pass: compute and apply gradient update using ADAM optimizer
    if (iter > GLOBAL_ITER_THRESH) {
      s1.start();
    }
